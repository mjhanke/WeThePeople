...
def convert_congress_bill(bill):
    ...
    new_bill['topic'] = '' if bill['subjects_top_term'] is None else bill['subjects_top_term'].replace(' ', '_')
    new_bill['subtopics'] = list(map(lambda sub : sub.replace(' ', '_'), bill.get('subjects', '')))
