...
export default class Swiper extends React.Component {
  ...
  onPanResponderMove = (event, gestureState) => {
    ...
    if (Math.abs(this.animatedValueX) > Math.abs(this.animatedValueY) && Math.abs(this.animatedValueX) > overlayOpacityHorizontalThreshold) {
      if (this.animatedValueX > 0) isSwipingRight = true;
      else isSwipingLeft = true;
    } else if (Math.abs(this.animatedValueY) > Math.abs(this.animatedValueX) && Math.abs(this.animatedValueY) > overlayOpacityVerticalThreshold) {
      if (this.animatedValueY > 0) isSwipingBottom = true;
      else isSwipingTop = true;
    }

    if (isSwipingRight) {
      this.setState({ labelType: LABELTYPES.RIGHT });
    } else if (isSwipingLeft) {
      this.setState({ labelType: LABELTYPES.LEFT });
    } else if (isSwipingTop) {
      this.setState({ labelType: LABELTYPES.TOP });
    } else if (isSwipingBottom) {
      this.setState({ labelType: LABELTYPES.BOTTOM });
    } else {
      this.setState({ labelType: LABELTYPES.NONE });
    }

    const { onTapCardDeadZone } = this.props;
    if (
      this.animatedValueX < -onTapCardDeadZone ||
      this.animatedValueX > onTapCardDeadZone ||
      this.animatedValueY < -onTapCardDeadZone ||
      this.animatedValueY > onTapCardDeadZone
    ) {
      this.setState({
        slideGesture: true,
  ...
  validPanResponderRelease = () => {
    ...
    const {
      ...
      isSwipingTop,
      isSwipingBottom,
    } = this.getSwipeDirection(this.animatedValueX, this.animatedValueY);
  ...
  calculateOverlayLabelStyle = () => {
    ...
    if (this.state.labelType === LABELTYPES.NONE) {
  ...
  renderOverlayLabel = () => {
    ...
    const labelTypeNone = labelType === LABELTYPES.NONE;
    const directionSwipeLabelDisabled =
      (labelType === LABELTYPES.BOTTOM && disableBottomSwipe) ||
      (labelType === LABELTYPES.LEFT && disableLeftSwipe) ||
      (labelType === LABELTYPES.RIGHT && disableRightSwipe) ||
      (labelType === LABELTYPES.TOP && disableTopSwipe);
...
Swiper.defaultProps = {
  ...
  inputRotationRange: [-width / 1.5, 0, width / 1.5],
  marginBottom: 0,
  marginTop: 0,
  onSwiped: (cardIndex) => {
