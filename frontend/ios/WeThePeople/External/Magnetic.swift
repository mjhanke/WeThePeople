...
open class Magnetic: SKScene {
    ...
    public lazy var magneticField: SKFieldNode = { [unowned self] in
        ...
        field.region = SKRegion(radius: 12000)
        field.minimumRadius = 12000
        field.strength = 3000
        self.addChild(field)
        return field
    }()
    ...
    func configPhysics() {
        ...
        magneticField.position = CGPoint(x: 0/*size.width / 2*/, y: size.height / 2)
    }
    
    override open func addChild(_ node: SKNode) {
        //var x = -node.frame.width // left
        //if children.count % 2 == 0 {
            let x = frame.width + node.frame.width // right
        //}
